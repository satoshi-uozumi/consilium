...
- Specialist consultation uses sub-agents — the sub-agent reads SKILL.md itself (local: Read tool; remote: get_skill MCP call); only the distilled result returns to main Claude; SKILL.md never enters the main context; cs:consult asks whether main Claude should pre-curate codebase context (files enter main context but sub-agents cost fewer tokens) or let sub-agents explore freely (main context stays clean)
