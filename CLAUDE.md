...
- Gateway-served and remote specialists are each their own McpServer at `/<name>`, exposing a single `get_skill` tool; Claude Code connects as `consilium-<name>:get_skill`; local (no-URL) specialists are read directly by sub-agents via the Read tool; all orchestration lives in the slash commands
...
- `consilium start` writes `consilium-<name>` MCP entries to `.claude/settings.json` for specialists with URLs only; starts local gateway if any URL points to localhost; `consilium stop` removes entries and stops the gateway
...
- Specialist consultation uses sub-agents — the sub-agent reads SKILL.md itself (local: Read tool; remote: get_skill MCP call); only the distilled result returns to main Claude; SKILL.md never enters the main context; cs:plan asks the user upfront (before reading any files) whether to pre-curate codebase context (main Claude reads files and passes them — faster, costs tokens here) or let sub-agents explore freely (main context stays clean)
