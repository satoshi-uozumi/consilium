...
4. Ask the user: "Should I pre-curate codebase context for sub-agents (I read the files here and pass them — faster, costs tokens in this session), or let sub-agents explore freely (main context stays clean, each sub-agent pays its own exploration cost)?" Wait for the answer before proceeding. If pre-curate: use Grep/Glob to find relevant files, read them, and include the content in each sub-agent prompt. If explore freely: skip this step entirely.
